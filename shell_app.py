...
def get_matrix_rain_html():
    ...
    for idx in range(total_columns):
        ...
        row_count = int(rng.integers(70, 130))
        chars_raw = rng.choice(char_bank, row_count)
        chars = [html.escape(c) for c in chars_raw]
        ...
        glyphs_html = f"<span class='matrix-head'>{head_char}</span>"
        if tail_chars:
            glyphs_html += "<br>" + "<br>".join(tail_chars)
            
        duration = rng.uniform(8.0, 14.0)
        ...
        column_spans.append(
            f"<span class=\"{depth_class}\" style=\"left:{left:.2f}vw; --duration:{duration:.2f}s; --delay:{delay:.2f}s; --intensity:{intensity:.2f}; will-change: transform, opacity;\">{glyphs_html}</span>"
        )
    ...
    <style>
        ...
        .matrix-rain::after {{
            ...
            position: absolute;
            inset: -20vh;
            background-image: repeating-linear-gradient(90deg, rgba(0, 32, 0, 0.35) 0px, rgba(0, 32, 0, 0.35) 2px, transparent 2px, transparent 8px);
            opacity: 0.14;
            filter: blur(6px);
            animation: matrixDrift 28s linear infinite;
        }}
        .matrix-rain::after {{
            opacity: 0.08;
            filter: blur(12px);
        ...
        .matrix-column {{
            ...
            text-shadow: 0 0 4px rgba(0,255,180,0.45),
                         0 0 12px rgba(0,255,110,0.3),
                         0 0 22px rgba(0, 120, 0, 0.55);
            mix-blend-mode: screen;
            filter: drop-shadow(0 0 4px rgba(0,255,140,0.25));
            opacity: 0;
        ...
        .matrix-head {{
            ...
            text-shadow: 0 0 8px rgba(200,255,200,0.7),
                         0 0 18px rgba(0,255,170,0.55);
            font-weight: 700;
        }}
        .matrix-column::before {{
            ...
            background: linear-gradient(180deg, rgba(255,255,255,0.25) 0%, rgba(0,255,120,0.1) 65%, transparent 100%);
            filter: blur(6px);
            opacity: 0;
            animation: matrixHeadGlow var(--duration, 12s) linear infinite;
            animation-delay: var(--delay, 0s);
        }}
        .matrix-column::after {{
            content: "";
            ...
            background: linear-gradient(180deg, rgba(0,255,120,0.35) 0%, rgba(0,0,0,0) 80%);
            filter: blur(9px);
            opacity: 0;
            animation: matrixTrail var(--duration, 12s) linear infinite;
            animation-delay: var(--delay, 0s);
        }}
        @keyframes matrixFall {{
            0% {{ transform: translateY(-140vh); opacity: 0; }}
            10% {{ opacity: var(--intensity, 0.65); }}
...
with st.sidebar:
    ...
    if st.session_state.theme == "matrix":
        ...
        st.session_state.matrix_rain_enabled = False
...
if not st.session_state.show_theory:
    ...
    with right_col:
        if generate:
            with st.spinner("Generating..."):
                ...
                t = np.linspace(0, st.session_state.num_turns*2*np.pi, 180)
                ...
                for i in range(theta.size):
                    for j in range(t.size):
                        pt = model(T[i,j], TH[i,j], st.session_state.b, st.session_state.d,
                                   z_used, st.session_state.a, phi, psi, delta,
                ...
                if thickness > 0:
                    dθ = theta[1]-theta[0]
                    dt = t[1]-t[0]
                    dX_dθ, dX_dt = np.gradient(X, dθ, dt)
                    dY_dθ, dY_dt = np.gradient(Y, dθ, dt)
                    dZ_dθ, dZ_dt = np.gradient(Z, dθ, dt)
                    Nx = dY_dθ*dZ_dt - dY_dt*dZ_dθ
                    Ny = dZ_dθ*dX_dt - dZ_dt*dX_dθ
                    Nz = dX_dθ*dY_dt - dX_dt*dY_dθ
                    N = np.sqrt(Nx**2 + Ny**2 + Nz**2 + 1e-20)
                    Nx, Ny, Nz = Nx/N, Ny/N, Nz/N

                    
                    if np.mean(X) > 0:
                        Nx, Ny, Nz = -Nx, -Ny, -Nz
                    inner_X = X - thickness * Nx
                    inner_Y = Y - thickness * Ny
                    inner_Z = Z - thickness * Nz
                ...
                if render_mode == "Surface":
                    ...
                    for i in range(X.shape[0]):
                        fig.add_trace(go.Scatter3d(x=X[i,:], y=Y[i,:], z=Z[i,:], mode='lines', line=dict(color=shell_color, width=1), showlegend=False))
                    for j in range(X.shape[1]):
                        fig.add_trace(go.Scatter3d(x=X[:,j], y=Y[:,j], z=Z[:,j], mode='lines', line=dict(color=shell_color, width=1), showlegend=False))
                    if thickness > 0:
                        for i in range(inner_X.shape[0]):
                            fig.add_trace(go.Scatter3d(x=inner_X[i,:], y=inner_Y[i,:], z=inner_Z[i,:], mode='lines', line=dict(color=shell_color, width=1), showlegend=False))
                        for j in range(inner_X.shape[1]):
                            fig.add_trace(go.Scatter3d(x=inner_X[:,j], y=inner_Y[:,j], z=inner_Z[:,j], mode='lines', line=dict(color=shell_color, width=1), showlegend=False))
    ...
    # ====================  STL  ====================
                ...
                if thickness > 0:
                    inner_verts = np.stack([inner_X.ravel(), inner_Y.ravel(), inner_Z.ravel()], axis=1).astype(np.float32)
                    verts = np.vstack([verts, inner_verts])
                
                offset = len(verts) // 2 if thickness > 0 else 0

                faces = []
                nt, nθ = t.size, theta.size
                for i in range(nθ):
                    i1 = (i + 1) % nθ
                    for j in range(nt - 1):
                        a = i * nt + j
                        ...
                        if thickness > 0:
                            ...
                            faces.append([a + offset, c + offset, d + offset])
                ...
                # Header 80 
                header = b"Mollusc shell generated by Ziyue Xu @ HKUST - 2025"
                stl_buffer.write(header.ljust(80, b" "))
                
                # triangle
                stl_buffer.write(struct.pack("<I", len(faces)))
                ...
                for tri in faces:
                    ...
                    # normal + v1 + v2 + v3 + attribute
                    stl_buffer.write(struct.pack("<3f", *normal))
                    stl_buffer.write(struct.pack("<3f", *v0))
                    stl_buffer.write(struct.pack("<3f", *v1))
                    stl_buffer.write(struct.pack("<3f", *v2))
                    stl_buffer.write(struct.pack("<H", 0))   # attribute byte count

                # =============== Generate png for download (vector with transparent background) ===============
                png_buffer = None
                ...
                if kaleido_available:
                    try:
                        ...
                        export_fig.write_image(png_buffer, format="png", engine="kaleido")
                        png_buffer.seek(0)
                    except Exception as err:
                        png_export_error = f"PNG export failed: {err}"
                else:
                    png_export_error = "PNG export requires the kaleido package (`pip install -U kaleido`)."
                ...
                with col_dl2:
                    if png_buffer:
                        ...
                        st.info(png_export_error or "PNG export is temporarily unavailable.")
        else:
