...
if not st.session_state.show_theory:
    ...
    with left_col:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.number_input("W (Whorl Expansion Rate)", min_value=0.0, step=0.01, format="%.4f", key="b")
            st.number_input("D (Distance from the Axis)", min_value=0.0, step=0.01, format="%.4f", key="d")
            st.number_input("T (Translation)", step=0.01, format="%.4f", key="z")
